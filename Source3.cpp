...
*/
	...
	//Filling the HistTable
	HistTable[0][2] = HistTable[0][1];
	for(int i=1;i<101;i++)
	{
		HistTable[i][2] = HistTable[i-1][2] + HistTable[i][1]; 
	}

	double multFactor = (double)101.0/HistTable[100][2];
	HistTable[0][3] = (HistTable[0][2]/2) * multFactor;  
	HistTable[0][4] = floor(HistTable[0][3]);

	for(int i=1;i<101;i++)
	{
		HistTable[i][3] = (double)(HistTable[i-1][2]+HistTable[i][2])/2 * multFactor; 
		HistTable[i][4] = floor(HistTable[i][3]);
		if(HistTable[i][4] > 100)
			HistTable[i][4] = 100;
